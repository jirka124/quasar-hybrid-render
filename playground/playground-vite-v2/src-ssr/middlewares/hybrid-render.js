...
import { RenderCSR, RenderSSG, RenderISR, RenderSWR } from '../../src-hr/Render.js'
...
export default ssrMiddleware(({ app, resolve, render, serve }) => {
  ...
  app.get(resolve.urlPath('*'), async (req, res, next) => {
    try {
      ...
      if (!req.hybridRender.route.type || req.hybridRender.route.type === 'ssr') return next()

      if (req.hybridRender.route.type === 'ssg' && hybridConf.SSG.actAsSSR) return next()
      if (req.hybridRender.route.type === 'isr' && hybridConf.ISR.actAsSSR) return next()
      if (req.hybridRender.route.type === 'swr' && hybridConf.ISR.actAsSSR) return next()
