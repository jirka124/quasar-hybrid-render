  ...
  async modifyEntry(html) {
    if (this.qa.hasWebpack) {
      ...
      if (this.qc.ctx.prod) {
        ...
        const jsFiles = await fs.readdir(path.join(this.distDir, "www/js"));
        const cssFiles = await fs.readdir(path.join(this.distDir, "www/css"));
...
class PrepareSSG {
  ...
  async startProdServer(shouldRunGenerate) {
    ...
    if (this.qc.ctx.prod && shouldRunGenerate) {
      ...
      const svr = require(path.join(this.distDir, "index.js"));

      server = this.qa.hasVite ? svr.default : svr;
    }
    return server;
  }
